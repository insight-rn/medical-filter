...
<<<<<<< HEAD
  ...
  mainTitle.innerText = "FILTER";

  if (stepId === 'root') {
    appSubtitle.classList.add('hidden');
    appSubtitle.innerText = "";
    mainDesc.innerText = "30초 필터링으로 내 케이스를 확인하고, 우리 병원의 정당한 수익을 지키세요.";
  } else if (appMode === 'bone') {
    appSubtitle.classList.remove('hidden');
    appSubtitle.innerText = "골밀도검사(BMD)";
    mainDesc.innerText = "순서대로 질문에 답하시면 급여 적용 여부를 확인해 드립니다.";
